...
//sp<IRkOutputManagerCallback> RkOutputManager::mCallback = nullptr;
...
Return<void> RkOutputManager::getCurCorlorMode(Display display, getCurCorlorMode_cb hidl_cb)
{
	...
	std::string tmp;
	tmp = hidl_mode.c_str();
	ALOGV("%s:%d color_mode:%s mCurMode=%s", __FUNCTION__, __LINE__, tmp.c_str(), mCurMode);
	return Void();
}
...
Return<void> RkOutputManager::getCurMode(Display display, getCurMode_cb _hidl_cb)
{
	...
	std::string tmp;
	tmp = hidl_mode.c_str();
	ALOGV("%s:%d hidl_mode:%s mCurMode=%s", __FUNCTION__, __LINE__, tmp.c_str(), mCurMode);
	return Void();
}
...
Return<void> RkOutputManager::getOverscan(Display display, getOverscan_cb _hidl_cb)
{
	...
	ALOGV("%s:%d hidl_overscan:%d %d %d %d", __FUNCTION__, __LINE__, hidl_overscan[0], hidl_overscan[1], hidl_overscan[2], hidl_overscan[3]);
	return Void();
}
...
IRkOutputManager* HIDL_FETCH_IRkOutputManager(const char* /* name */) {
	...
	ALOGD("HIDL_FETCH_IRkOutputManager ret=%d %d(%s)", ret, errno, strerror(errno));
    if (ret == 0) {
